...
const mettreAJourStatutParking = async (client, idParking) => {
  ...
  const couleur = statut === 'libre' ? '#10B981' : statut === 'quasi-plein' ? '#F97316' : '#EF4444';
  await client.query(
...
const creerReservation = async (req, res) => {
  ...
  try {
    const { id_user, id_parking, heure_debut, heure_fin, date_reservation } = req.body;
    if (!id_user || !id_parking || !heure_debut || !heure_fin) {
      return res.status(400).json({ success: false, message: 'Champs obligatoires : id_user, id_parking, heure_debut, heure_fin' });
    }
    ...
    if (!parkingRes.rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ success: false, message: 'Parking introuvable' }); }
    const parking = parkingRes.rows[0];
    if (parking.statut === 'complet') { await client.query('ROLLBACK'); return res.status(409).json({ success: false, message: 'Ce parking est complet' }); }

    const userRes = await client.query('SELECT id_user FROM utilisateurs WHERE id_user = $1', [id_user]);
    if (!userRes.rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ success: false, message: 'Utilisateur introuvable' }); }

    const prix = calculerPrix(heure_debut, heure_fin, parking.prix);
    if (!prix) { await client.query('ROLLBACK'); return res.status(400).json({ success: false, message: 'heure_fin doit être après heure_debut' }); }

    let codeQR, tentatives = 0;
    do {
      codeQR = genererCodeQR();
      const existing = await client.query('SELECT id_reservation FROM reservations WHERE code_qr = $1', [codeQR]);
    ...
    await client.query('UPDATE parkings SET place_occupee = place_occupee + 1 WHERE id = $1', [id_parking]);
    await mettreAJourStatutParking(client, id_parking);
    await client.query('COMMIT');

    res.status(201).json({ success: true, message: 'Réservation créée avec succès', data: { ...rows[0], parking_nom: parking.nom } });
  } catch (err) {
...
const getReservationsUser = async (req, res) => {
  try {
    ...
    if (statut) { values.push(statut); query += ` AND r.statut = $${values.length}`; }
    query += ` ORDER BY r.date_reservation DESC, r.heure_debut DESC`;
    const { rows } = await pool.query(query, values);
    res.json({ success: true, data: rows });
  } catch (err) {
...
const getReservationById = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ success: false, message: 'Réservation introuvable' });
...
const getReservationByQR = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ success: false, message: 'QR code invalide' });
    res.json({ success: true, data: rows[0] });
  } catch (err) {
...
const terminerReservation = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    await client.query('BEGIN');
    const { rows } = await client.query('SELECT * FROM reservations WHERE id_reservation = $1 FOR UPDATE', [id]);
    if (!rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ success: false, message: 'Réservation introuvable' }); }
    const resa = rows[0];
    if (resa.statut !== 'en_cours') { await client.query('ROLLBACK'); return res.status(409).json({ success: false, message: `Impossible : la réservation est déjà "${resa.statut}"` }); }
    await client.query("UPDATE reservations SET statut = 'terminee' WHERE id_reservation = $1", [id]);
    await client.query('UPDATE parkings SET place_occupee = GREATEST(place_occupee - 1, 0) WHERE id = $1', [resa.id_parking]);
...
const annulerReservation = async (req, res) => {
  ...
  try {
    ...
    const resa = rows[0];
    if (resa.statut !== 'en_cours') { await client.query('ROLLBACK'); return res.status(409).json({ success: false, message: `Impossible : la réservation est déjà "${resa.statut}"` }); }
    await client.query("UPDATE reservations SET statut = 'annulee' WHERE id_reservation = $1", [id]);
    await client.query('UPDATE parkings SET place_occupee = GREATEST(place_occupee - 1, 0) WHERE id = $1', [resa.id_parking]);
    await mettreAJourStatutParking(client, resa.id_parking);
    await client.query('COMMIT');
